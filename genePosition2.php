...
=======
	...
	$array_reult=array();
	//traverse three species to find gene name
	foreach($array_species as $spe=>$database) {
		if(strcmp($species,$spe) == 0) {	//judge input species equals which database
			$sql1="select * from $database.multishade where chrom='$chromosome' AND chromStart>='$geneStart' AND chromEnd<='$geneEnd'";
			$result = mysql_query($sql1,$mysqlcon);
			
			if(mysql_num_rows($result)) {
				while($row1 = mysql_fetch_array($result)) {
					$str1 = "$database".": chrom:$row1[chrom] chromStart:$row1[chromStart] chromEnd:$row1[chromEnd] name:$row1[name]<br/>";
					echo $str1;
					$str_spe1="$row1[chrom]:$row1[chromStart]-$row1[chromEnd]:$row1[name]";
			
			
					//acquire other species database to later query
					foreach($array_species as $spec=>$data) {
						if(strcmp($species,$spec) != 0) {
							$array_spec = array("$data");
						}
					}
			
			
					$sql2 = "select * from $array_spe[1].multishade where name='$row1[name]'";
					$result_spe2 = mysql_query($sql2,$mysqlcon);
					if(mysql_num_rows($result_spe2)){
						while($row2 = mysql_fetch_array($result_spe2)){
							$str2 = "specy".": chrom:$row2[chrom] chromStart:$row2[chromStart] chromEnd:$row2[chromEnd] name:$row2[name]<br/>";
							echo $str2;	
							$str_spe2="$row2[chrom]:$row2[chromStart]-$row2[chromEnd]:$row2[name]";
						}
					}else{
						$str_spe2="Not found $row1[name]";
						echo "$array_spe[1]".": Not found $row1[name] <br/>";
					}
		
					$sql3 = "select * from $array_spe[2].multishade where name='$row1[name]'";
					$result_spe3 = mysql_query($sql3,$mysqlcon);
					if(mysql_num_rows($result_spe3)){
						while($row3 = mysql_fetch_array($result_spe3)){
							$str3 = "Scr2: chrom:$row3[chrom] chromStart:$row3[chromStart] chromEnd:$row3[chromEnd] name:$row3[name]<br/>";
							echo $str3;
							$str_spe3="$row3[chrom]:$row3[chromStart]-$row3[chromEnd]:$row3[name]";
						}
					}else{
						$str_spe3="Not found $row1[name]";
						echo "$array_spe[2]",": Not found $row1[name] <br/>";
					}
...
/*	
	$sql1="select * from hg19.multishade where chrom='$chromosome' AND chromStart>='$geneStart' AND chromEnd<='$geneEnd'";
>>>>>>> ce9a98dba60cb0b164eb4e3348f2d811ca80587e
	$result = mysql_query($sql1,$mysqlcon);
